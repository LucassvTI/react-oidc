...
const loginCallbackWithAutoTokensRenewAsync = async oidc => {
  ...
  oidc.timeoutId = await autoRenewTokensAsync(
    oidc,
    tokens.refreshToken,
...
const autoRenewTokensAsync = async (oidc, refreshToken, expiresAt) => {
  const refreshTimeBeforeTokensExpirationInSecond =
    oidc.configuration.refresh_time_before_tokens_expiration_in_second ?? 60;
  return timer.setTimeout(async () => {
    ...
    if (
      currentTimeUnixSecond >
      expiresAt - refreshTimeBeforeTokensExpirationInSecond
    ) {
...
const userInfoAsync = async oidc => {
  ...
  const oidcServerConfiguration = await oidc.initAsync(
    oidc.configuration.authority,
    oidc.configuration.authority_configuration
  );
...
const setTokensAsync = async (serviceWorker, tokens) => {
  ...
  return {
    ...tokens,
    idTokenPayload: idTokenPayload(tokens.idToken),
    accessTokenPayload,
    expiresAt,
  };
};
...
Please checkout that you are using OIDC hook inside a <OidcProvider configurationName="${name}"></OidcProvider> compoment.`);
  ...
  silentSigninCallbackFromIFrame() {
    if (this.configuration.silent_redirect_uri) {
      window.top.postMessage(
        `${this.configurationName}_oidc_tokens:${JSON.stringify(this.tokens)}`,
  ...
  async silentSigninAsync() {
    ...
    const promise = new Promise((resolve, reject) => {
      try {
        ...
        const silentSigninTimeout = configuration.silent_signin_timeout
          ? configuration.silent_signin_timeout
          : 12000;
        setTimeout(() => {
          if (!isResolved) {
            reject("timeout");
            self.publishEvent(
              eventNames.silentSigninAsync_error,
              new Error("timeout")
            );
            iframe.remove();
            isResolved = true;
      ...
      try {
        ...
        if (serviceWorker) {
          ...
          if (tokens) {
            ...
            const updatedTokens = await this.refreshTokensAsync(
            ...
            // @ts-ignore
            this.timeoutId = await autoRenewTokensAsync(
              this,
  ...
  async loginAsync(
    ...
    try {
      ...
      sessionStorage[`oidc_login.${this.configurationName}`] = JSON.stringify({
        callbackPath: url,
        extras,
        state,
      });

      let serviceWorker = await initWorkerAsync(
        configuration.service_worker_relative_url,
        this.configurationName
      );
      const oidcServerConfiguration = await this.initAsync(
      ...
      if (serviceWorker && installServiceWorker) {
        const isServiceWorkerProxyActive =
          await serviceWorker.isServiceWorkerProxyActiveAsync();
  ...
  async refreshTokensAsync(refreshToken, silentEvent = false) {
    const localSilentSigninAsync = async (exception = null) => {
      ...
      this.publishEvent(
        silentEvent
          ? eventNames.refreshTokensAsync_silent_error
          : eventNames.refreshTokensAsync_error,
        exception
      );
      return null;
    };
    ...
    try {
      this.publishEvent(
        silentEvent
          ? eventNames.refreshTokensAsync_silent_begin
          : eventNames.refreshTokensAsync_begin,
        {}
      );
      const configuration = this.configuration;
      const clientId = configuration.client_id;
      ...
      if (
        !configuration.scope.split(" ").find(s => s === refresh_token_scope)
      ) {
      ...
      const token_response = await tokenHandler.performTokenRequest(
        oidcServerConfiguration,
        request
      );
      this.publishEvent(
        silentEvent
          ? eventNames.refreshTokensAsync_silent_end
          : eventNames.refreshTokensAsync_end,
        token_response
      );
  ...
  async logoutAsync(
    ...
    if (postLogoutRedirectUri && typeof postLogoutRedirectUri !== "string") {
      ...
      console.warn("postLogoutRedirectUri url is not a string");
    }
    const path =
      callbackPath === null || callbackPath === undefined
        ? location.pathname + (location.search || "") + (location.hash || "")
        : callbackPath;
    const url =
      postLogoutRedirectUri === undefined
    ...
    if (oidcServerConfiguration.endSessionEndpoint) {
      ...
      window.location.href = `${
        ...
        url
      )}&id_token_hint=${idToken}${extraQueryString}`;
    } else {
